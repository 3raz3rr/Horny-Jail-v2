SUBSYSTEM_DEF(air)
	...
	var/map_loading = TRUE
        ...
        /// buffer of recent debug messages for on-demand export
        var/list/debug_buffer = list()
...
/datum/controller/subsystem/air/proc/debug_log(message)
        if(!debug_enabled)
                return
        var/line = "[time2text(world.timeofday)]: [message]"
        world.log << line
        debug_buffer += line
        if(debug_buffer.len > 1000)
                debug_buffer.Cut(1, debug_buffer.len - 1000)
...
/// dump accumulated debug log lines to a file and offer it for download
/datum/controller/subsystem/air/proc/export_debug_log(mob/requester)
        if(!istype(requester))
                return
        if(!debug_buffer.len)
                tgui_alert(requester, "No debug logs recorded", "Atmos Debug")
                return
        var/file_name = "air_debug_log_[time2text(world.timeofday, \"MMM_DD_YYYY_hh-mm-ss\", TIMEZONE_UTC)].log"
        var/temp_path = "[GLOB.log_directory]/[file_name]"
        if(!text2file(jointext(debug_buffer, "\n"), temp_path))
                tgui_alert(requester, "Failed to export air log", "Atmos Debug")
                return
        var/export_file = file(temp_path)
        requester << ftp(export_file, file_name)
        fdel(temp_path)
